...
public class UsersService {
    ...
    public boolean createOne(User user) {
        ...
        if(authenticationProxy.createOne(user.getPseudo(), new UnsafeCredentials(user.getPseudo(), user.getPassword())).getStatusCode().isError()) return false;
    ...
    public boolean updateOne(User user) {
        if (!repository.existsById(user.getPseudo())) return false;
        authenticationProxy.updateOne(user.getPseudo(), new UnsafeCredentials(user.getPseudo(), user.getPassword()));
        repository.save(user);
        return true;
    }
